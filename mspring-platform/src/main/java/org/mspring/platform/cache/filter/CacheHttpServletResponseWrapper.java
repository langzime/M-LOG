...
public class CacheHttpServletResponseWrapper extends HttpServletResponseWrapper {
    ...
    private static final Logger log = Logger.getLogger(CacheHttpServletResponseWrapper.class);
    ...
    private long expires = CachingFilter.EXPIRES_ON;
    private long lastModified = CachingFilter.LAST_MODIFIED_INITIAL;
    ...
    public CacheHttpServletResponseWrapper(HttpServletResponse response) {
        this(response, false, Long.MAX_VALUE, CachingFilter.EXPIRES_ON, CachingFilter.LAST_MODIFIED_INITIAL, -60);
    }
     ...
     *            The servlet response
     * @param fragment
     *            true if the repsonse indicates that it is a fragement of a
     *            page
     * @param time
     *            the refresh time in millis
     * @param lastModified
     *            defines if last modified header will be send, @see
     *            CachingFilter
     * @param expires
     *            defines if expires header will be send, @see CachingFilter
     * @param cacheControl
     *            defines if cache control header will be send, @see
     *            CachingFilter
     */
    public CacheHttpServletResponseWrapper(HttpServletResponse response, boolean fragment, long time, long lastModified, long expires, long cacheControl) {
        ...
        // only set inital values for last modified and expires, when a complete
        // page is cached
        if (!fragment) {
            // setting a default last modified value based on object creation
            // and remove the millis
            if (lastModified == CachingFilter.LAST_MODIFIED_INITIAL) {
                long current = System.currentTimeMillis();
                current = current - (current % 1000);
                result.setLastModified(current);
                super.setDateHeader(CachingFilter.HEADER_LAST_MODIFIED, result.getLastModified());
            }
            // setting the expires value
            if (expires == CachingFilter.EXPIRES_TIME) {
                result.setExpires(result.getLastModified() + time);
                super.setDateHeader(CachingFilter.HEADER_EXPIRES, result.getExpires());
            }
            // setting the cache control with max-age
            if (this.cacheControl == CachingFilter.MAX_AGE_TIME) {
                // set the count down
                long maxAge = System.currentTimeMillis();
                maxAge = maxAge - (maxAge % 1000) + time;
                result.setMaxAge(maxAge);
                super.addHeader(CachingFilter.HEADER_CACHE_CONTROL, "max-age=" + time / 1000);
            } else if (this.cacheControl != CachingFilter.MAX_AGE_NO_INIT) {
                result.setMaxAge(this.cacheControl);
                super.addHeader(CachingFilter.HEADER_CACHE_CONTROL, "max-age=" + (-this.cacheControl));
            } else if (this.cacheControl == CachingFilter.MAX_AGE_NO_INIT) {
                result.setMaxAge(this.cacheControl);
            }
        }
    ...
    public void setDateHeader(String name, long value) {
        ...
        if ((lastModified != CachingFilter.LAST_MODIFIED_OFF) && (CachingFilter.HEADER_LAST_MODIFIED.equalsIgnoreCase(name))) {
            if (!fragment) {
                result.setLastModified(value);
            } // TODO should we return now by fragments to avoid putting the
              // header to the response?
        }

        // implement RFC 2616 14.21 Expires (without max-age)
        if ((expires != CachingFilter.EXPIRES_OFF) && (CachingFilter.HEADER_EXPIRES.equalsIgnoreCase(name))) {
            if (!fragment) {
    ...
    public void setHeader(String name, String value) {
        ...
        if (CachingFilter.HEADER_CONTENT_TYPE.equalsIgnoreCase(name)) {
            result.setContentType(value);
        }

        if (CachingFilter.HEADER_CONTENT_ENCODING.equalsIgnoreCase(name)) {
    ...
    public void reset() {
        ...
        super.reset();
        /*
         * cachedWriter = null; result = new ResponseContent(); cacheOut = null;
         * fragment = false; status = SC_OK; expires = CachingFilter.EXPIRES_ON;
         * lastModified = CachingFilter.LAST_MODIFIED_INITIAL; cacheControl =
         * -60; // time ?
         */
    }
    ...
    public void resetBuffer() {
        ...
        super.resetBuffer();
        /*
         * //cachedWriter = null; result = new ResponseContent(); //cacheOut =
         * null; //fragment = false;
         */
    }
