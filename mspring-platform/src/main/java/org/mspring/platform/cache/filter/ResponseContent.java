...
public class ResponseContent implements Serializable {
     ...
     * 
     * @param value
     *            time in miliseconds when the content will expire
     */
     ...
     *            The servlet response to output the cached content to.
     * @param fragment
     *            is true if this content a fragment or part of a page
     * @param acceptsGZip
    ...
    public void writeTo(ServletResponse response, boolean fragment, boolean acceptsGZip) throws IOException {
        ...
        if (fragment) {
            ...
            if (response instanceof HttpServletResponse) {
                ...
                if (lastModified != -1) {
                    httpResponse.setDateHeader(CachingFilter.HEADER_LAST_MODIFIED, lastModified);
                }

                // add the expires header
                if (expires != Long.MAX_VALUE) {
                    httpResponse.setDateHeader(CachingFilter.HEADER_EXPIRES, expires);
                }

                // add the cache-control header for max-age
                if (maxAge == CachingFilter.MAX_AGE_NO_INIT || maxAge == CachingFilter.MAX_AGE_TIME) {
                    // do nothing
                    ...
                    httpResponse.addHeader(CachingFilter.HEADER_CACHE_CONTROL, "max-age=" + currentMaxAge);
                } else {
                    httpResponse.addHeader(CachingFilter.HEADER_CACHE_CONTROL, "max-age=" + (-maxAge));
                }
        ...
        if (isContentGZiped()) {
            if (acceptsGZip) {
                ((HttpServletResponse) response).addHeader(CachingFilter.HEADER_CONTENT_ENCODING, "gzip");
